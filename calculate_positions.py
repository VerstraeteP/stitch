...
def calculate_pos(renners,Affinetransform,aantalrenners,afbeelding,fps_scaled,fps,total_transform,indexen,width):
	...
	for index in range(len(Affinetransform)):
		...
		if index>=2:
			...
			line =np.array([[[line[0][0][0]+offset,line[0][0][1]+300],[line[0][1][0]+offset,line[0][1][1]+300]]], dtype = "float32")
		...
		for k in track[index]:
			if int(k[4])<=aantalrenners:
				...
				if index>=2:
					...
					pts =np.array([[[(k[0]+(k[2]-k[0])/2)+offset,(k[1]+(k[3]-k[1])/2)+300]]], dtype = "float32")
		...
		renner=[]
	transformaties=Affinetransform.to_list()
	...
	for k in linearray[0]:
		cv2.line(afbeelding, (int(k[0][0]),int(k[0][1])), (int(k[1][0]),int(k[1][1])), colors[4], 2)
